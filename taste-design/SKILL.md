...
Use this mode when the task is to abstract an interface rather than fully render it: placeholder mockups, skeleton previews, empty or future states, onboarding or marketing frames that depict product UI, design-system storytelling, flows told as a sequence of screens, or any moment when real data does not exist or should not carry the design.

Baby UI keeps every structural rule in this skill and removes content detail. The result must read as a calm, finished miniature of the product — not as a wireframe, a loading-state dump, or an unfinished design. Abstraction lives in the content; finish quality stays at full strength.

Core abstraction directive:

1. Greek the data, never the structure. Layout, hierarchy, spacing, radius, shadow layering, and the single accent all remain real.
2. Classify every text element as narrative or data:
   - Narrative survives as real text: the state headline ("Title would like to stay"), structural labels ("Check-in", "Guests"), section markers ("Section"), eyebrows, and action verbs.
   - Data collapses into bars: values, dates, amounts, counts, notes, body copy, and metadata.
3. Never greek a primary action label. Buttons keep real verbs.
4. Keep at least one real narrative line per screen as the anchor. A screen of only bars reads as a loading skeleton, not a baby UI.
5. Apply content neutrality rules to surviving text: use the user's content when provided, otherwise short neutral words. Do not invent names to fill identity slots.

Skeleton bars:

1. Build bars as fully rounded pills with a flat single gray tone, roughly `#E8E9E6` to `#EDEDEB` on white surfaces, one value step darker than secondary panels.
2. Match bar height to the optical x-height of the text it replaces, roughly 8–12 px at mobile scale for body text.
3. Vary bar widths to imply language: first line longer, second line roughly 55–75% of the first. Do not stack equal-width bars.
4. Use at most two bars per text block. A paragraph stand-in is two bars, not five.
5. Align bars to the text column they replace. Bars inherit the alignment of the type they stand in for: left rail in lists, right-aligned for trailing values.
6. For label-value rows, keep the real gray label on the left and one short bar on the right.
7. Give bars no borders, no gradients, no shimmer, and no animation. This is static abstraction, not a loading indicator.

Block placeholders:

1. Collapse large media or content regions into one flat pale-gray rounded block, completely empty inside.
2. Use no icon, no X, no image glyph, no dashed border, and no caption inside the block.
3. Tone the block one value step darker than its parent surface and keep the same radius language as sibling cards.
4. Use at most one large block per screen and let it be the quietest large mass on the canvas.

Identity and badges:

1. Avatars are the strongest content survivors: a circular photo crop or a neutral gray initial circle.
2. Show overflow as a small pale circle with a short count chip such as "+3".
3. Keep status or verification badges tiny, attached to the avatar edge, and allow them to carry the accent hue.

Navigation and chrome:

1. Bottom navigation may reduce to evenly spaced small dots: gray filled dots for inactive items.
2. Mark the active item with a thin accent ring or a single accent dot, not a filled accent pill.
3. Reduce top chrome to at most two small circular controls. At most one may be accent-filled.

Accent under abstraction:

1. The single accent hue survives abstraction and reads relatively stronger because everything else is quiet: a progress bar, an active ring, one badge, or one filled circular control.
2. Keep total accent area within the same 2–5% budget. Abstraction is not a license to add color.

State storytelling:

1. When showing a flow or lifecycle, repeat an identical skeleton across frames and change only the narrative line, the accent state, and one or two content slots.
2. Sameness between frames is what tells the story. Do not redesign the layout per frame.
3. Order frames so the accent shows progression: badge, then progress bar, then completed state, for example.

Baby UI guardrails:

1. Do not collapse into wireframe: no dashed strokes, no placeholder X boxes, no mountain-and-sun image glyphs, no annotation arrows, no gray text reading "image here".
2. Do not use lorem ipsum, blurred text, or scribble strokes. Abstraction is bars, not garble.
3. Do not greek everything. Headlines, labels, and actions stay real.
4. Do not turn bars into sharp rectangles or give them outlines.
5. Do not stack more than three bar groups in a single support zone. Collapse remaining rows into whitespace instead.
6. Do not degrade shadows, radii, or spacing to look "sketchy". Baby UI is a polished artifact at reduced information density.

Baby UI checklist:

1. Does every frame keep at least one real narrative line?
2. Are all action labels real verbs?
3. Are bars pill-shaped, flat, single-tone, max two per block, with varied widths?
4. Are large content regions one quiet empty block, not a glyphed placeholder?
5. Do avatars, badges, and the single accent survive while data collapses?
6. Across a multi-frame story, is the skeleton identical and only the state changing?
7. Would the frame still read as the finished product seen from far away?
